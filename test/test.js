...
if (!args.mcworld) throw Error('Argument for "mcworld" not supplied. This should be the path to your minecraft world.')
if (!args.input) throw Error('Argument for "input" not supplied. This should be the path the json file created by Text2Book.')
...
let playerdata;
try {
    playerdata = fs.readFileSync(globSync(path.join(args.mcworld,'playerdata') + '/*.dat')[0])
} catch(e) {
    throw Error('Failed to find player inventory file, did you input the correct folder path?')
}
...
nbt.parse(playerdata, function (error, data) {
    ...
    if (targetBook === null) {
        throw Error('Could not find a written book with the title "text2book" in the players inventory.')
    }
    ...
    pages.forEach((page, index) => {
        const io = {expected: inputfile[index], got: page}
    ...
    if(results.fail.length > 0) {
        ...
        console.log(results.fail.length + '/' + pages.length + ' pages did not match output.\n');

        results.fail.forEach((item, index) => {
            console.log(`P${index} - Expected: \x1b[33m${item.expected}\x1b[0m, Got: \x1b[31m${removeQuotes(item.got)}\x1b[0m`);
        });
